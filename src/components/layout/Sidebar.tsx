...
export default function Sidebar({ mobileOpen = false, onCloseMobile }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`w-72 bg-[#0B0F19] border-r border-[#1F293D] flex flex-col h-screen fixed left-0 top-0 z-50 transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="h-20 px-6 flex items-center justify-between border-b border-[#1F293D] bg-[#0E1424]">
          ...
          {onCloseMobile && (
            <button
              onClick={onCloseMobile}
              className="lg:hidden p-1.5 rounded-xl text-gray-400 hover:text-white hover:bg-[#161F30] transition-colors"
